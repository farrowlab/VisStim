function ShowMovie(window,vparams,sparams);
%(moviename, fromTime, toTime, indexisFrames, benchmark, async, preloadSecs, specialflags, pixelFormat)
%
...
% 17/06/15  kf  Modified ShowMovieDemo for presentinator in FarrowLab
...
indexisFrames = 0;
benchmark = 0;
async = 0;            % set to 4 for preloading
preloadSecs = 5;
specialflags = 1;
pixelFormat = [];     % Need to think about this.
...
try    
    ...
    background = [vparams.BgColour, vparams.BgColour, vparams.BgColour];
    ...
    info.movie = movie;
    info.movieduration = movieduration;
    info.fps = fps;
    info.imgw = imgw;
    info.imgh = imgh;
...
% Ok, now the requested part of the movie has been (hopefully) loaded and converted into standard PTB textures. We can simply use the 'DrawTexture' command in a loop to show the textures...
...
%-----------------  Show Movie --------------------%
  ...
  TTLfunction(startbit,recbit); WaitSecs(.1); 
  if vparams.Calcium
    Screen('FillRect', window, [0 0 vparams.Background])
  else
  ...
  tstart=GetSecs;
    ...
    n = n + 1;
        ...
        Screen('DrawTexture', win, texids(currentindex),[], [105 5 695 595]);
        ...
        if (benchmark==0)
          ...
          if mod(currentindex,640)==0 % Send Stimbit
          end
