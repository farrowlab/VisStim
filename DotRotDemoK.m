...
try
    ...
    dot_speed   = 0;    % dot speed (deg/sec)
    ndots       = 10; % number of dots
    max_d       = 1;   % maximum radius of  annulus (degrees)
    min_d       = 0;    % minumum
    dot_w       = .2;  % width of dot (deg)
    fix_r       = .01; % radius of fixation point (deg)
    ...
    differentsizes = 10; % Use different sizes for each point if >= 1. Use one common size if == 0.
