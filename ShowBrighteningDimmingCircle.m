function StimLog = ShowBrighteningDimmingCircle(window, vparams, sparams)
...
% ---------------------- %
% --- Log parameters --- %
StimLog.StimulusClass = 'Brightening Dimming Circle';   
...
for k = 1:trial
  ...
  for j = 1:size(speedlist,2)
    ...
    % Dimming
    dim_lumlist = linspace(white,gray,nframes);    
    % Brightening
    bri_lumlist = linspace(gray,white,nframes);
    % White
    white_lumlist = white*ones(1,framerate*5); % stay for 5 sec
    ...
    for i = 1:(framerate*5)
      ...
      dstRect = [0 0 xsize ysize];
      dstRect = CenterRectOnPointd(dstRect, xCenter, yCenter);
      Screen('FillOval', window, [white_lumlist(i) white_lumlist(i) white_lumlist(i)], dstRect);
